...
proton()
j_ins_login <- employees[employees$surname == "Insecure" , 'login']
proton(action = "login", login=j_ins_login)
...
tmp[tmp>0]
host_most_often <- sort(tmp, decreasing = TRUE)
proton(action = 'server', host = names(host_most_often))
