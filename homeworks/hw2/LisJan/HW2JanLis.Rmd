...
plot1<-df %>% select(math.score, reading.score,writing.score) %>% 
  ...
  labs(title="Rozkład wyników z przedmiotów: matematyka, czytanie i pisanie",
       x="wynik",y="liczność")
plot1
...
plot6<-df %>% select(math.score,reading.score,writing.score) %>%
  ...
  geom_point() + 
  labs(title = "Wykres zależności wyników z matematyki od wyników z czytania oraz pisania.",
