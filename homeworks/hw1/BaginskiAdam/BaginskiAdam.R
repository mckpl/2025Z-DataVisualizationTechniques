...
## Rozwiązanie
x <- as.Date(reviews$date)
y <- as.numeric(format(x, "%y%m"))
y_lata <- as.numeric(format(x, "%Y"))

y <- y[y < 1511] #1511 to listopad 2015, czyli szukamy tych ktora maja recenzje z wcześniejszych okresów

odp_4 <- c(length(y), 2025 - min(y_lata))
