...
wojewodztwa %>% 
  ...
  ggplot() + 
  geom_sf(aes(fill = ogółem.2024....),
          color = "white") + 
  theme_void() +
  scale_fill_gradient(low = "lightblue", high = "navy") + 
  labs(title = "Zdawalność egzaminów maturalnych w roku 2024",
       fill = "%") 


miasta <- read.csv("miasta.csv")
miasta <- miasta %>% 
  filter(Miasto..Stolica. != 'Gorzów Wielkopolski',
         Miasto..Stolica. != 'Toruń')
miasta$Województwo <- tolower(miasta$Województwo)
...
wojewodztwa %>% 
  ...
  labs(title = "Zdawalność egzaminów maturalnych w roku 2024",
       fill = "%") +
  geom_text(data = miasta, aes(y = Szerokość.Geograficzna..N., x = Długość.Geograficzna..E., label = Województwo),
...
wojewodztwa %>% 
  left_join(dane, by = join_by(JPT_NAZWA_ == Nazwa)) %>% 
  left_join(miasta, by = join_by(JPT_NAZWA_ == Województwo)) %>% 
  ggplot() + 
  geom_sf(aes(fill = ogółem.2024....),
          color = "white") + 
  theme_void() +
  ...
  labs(title = "Zdawalność egzaminów maturalnych w roku 2024",
       fill = "%") +
  geom_text(aes(y = Szerokość.Geograficzna..N., x = Długość.Geograficzna..E., label = paste0(ogółem.2024...., "%")),
            position = position_dodge(1.2),
            vjust = 1,
            size = 5,
            fontface = "bold")
